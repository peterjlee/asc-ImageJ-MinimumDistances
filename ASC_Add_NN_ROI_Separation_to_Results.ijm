/*
	A brute force macro to measure the minimum separation between objects.
	...
	*/
	...
	if (createLegend){
		...
		if (legendFit == "Height"){
			...
			legendWidth = fontSize * 4;
			extraLegendLines = floor(maxLines*1.5*fontSize/imageHeight);
			legendWidth += extraLegendLines*fontSize*5;
		}else {
		...
		if (legendFit == "Height"){
			for(lineP=0;lineP<maxLines; lineP++){
				...
				drawLine(xStart,yStart,xStart+2*fontSize,yStart);
				drawString(lineP+1,xStart+2.5*fontSize,yStart + 0.75*fontSize);
				yStart += 1.5*fontSize;
		...
		if (legendFit == "Width"){
			for(lineP=0;lineP<maxLines; lineP++){
				setLineWidth(lineThickness[lineP]);
				setColorFromColorName(lineColors[lineP]);
				drawLine(xStart,yStart-fontSize/2,xStart+2*fontSize,yStart-fontSize/2);
				drawString(lineP+1,xStart+2.5*fontSize,yStart);
				xStart += 5*fontSize;
				if(xStart+5*fontSize>=imageWidth){
					xStart = 0.5*fontSize;
					yStart += 1.5*fontSize;
				}
			}
